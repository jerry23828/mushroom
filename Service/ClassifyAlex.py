...
from Database_config import db_init as mr
from Model.Records import Records
from datetime import datetime
class ImageClassifier:
    def __init__(self, model_path='./AI/mushroom_alex_32_0.0001.mod', img_dir="./dataset", batch_size=16, num_classes=10):
    ...
    def classify_image(self, img_path, user_id):
        ...
        # Save to database
        confidence_value = float(confidence.item()) if isinstance(confidence, torch.Tensor) else float(confidence)
        record = Records(
            user_id=user_id,
